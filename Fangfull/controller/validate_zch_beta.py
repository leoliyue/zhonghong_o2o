...
## 新奇世界老用户
    ...
    customerid = ['108953']
    cellphone = ['13000001326']
    getxls = openxls()
    maxThread = 8
    sem=threading.BoundedSemaphore(maxThread-1)
    for i in range(len(cellphone)):
        sem.acquire()
        t = RequestThread(req_session,bese_str,cellphone[i],'602',getxls,i,len(cellphone),'新奇世界13000001324',customerid[i]).start()
